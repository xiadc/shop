...
$(function(){
	...
	$("#registerForm").submit(function(){
		...
		var captchaVal = $("#captcha").val();//验证码
		
		if(usernameVal==null||usernameVal==""){
			alert("用户名不能为空！");
			return false;
		}else if(usernameVal.length < 3||usernameVal.length > 20){
			alert("用户名长度应该大于2且小于20");
			return false;
		}else if(passwordVal==null||passwordVal == ""){
			alert("密码不能为空！");
			return false;
		}else if(passwordVal.length < 3||passwordVal.length > 20){
			alert("密码长度必须大于2且小于20");
			return false;
			...
			alert("email不能为空！");
			return false;
		}else if(!/^([a-zA-Z0-9_-])+@([a-zA-Z0-9_-])+((\.[a-zA-Z0-9_-]{2,3}){1,2})$/.test(emailVal)){
			alert("email格式不正确");
			return false;
		}else if(isNaN(phone&&phoneVal)){
		    alert("电话号码必须为数字");
		    return false;
		}else if(!captchaVal){
			alert("验证码不能为空！");
